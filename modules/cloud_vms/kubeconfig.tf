locals {
  k3s_ssh_target = coalesce(var.k3s_mgmt_ip, var.k3s_server_ip)
  k3s_ssh_prefix = var.k3s_mgmt_ip != null ? "" : "-J root@${var.proxmox_ip}"
  k3s_api_ip     = var.k3s_server_ip
}
...
resource "terraform_data" "k3s_kubeconfig" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      ssh -o StrictHostKeyChecking=no ${local.k3s_ssh_prefix} ubuntu@${local.k3s_ssh_target} \
          'sudo cat /etc/rancher/k3s/k3s.yaml' \
        | sed "s|127.0.0.1|${local.k3s_api_ip}|g" \
